...
class Country extends React.Component {
  ...
  comma = (confirmedCases, newCases, deaths, newDeaths, recovered, active, critical, casesPerM, deathsPerM) => {
    this.setState({
      ...
      newDeaths: `${newDeaths ? newDeaths.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0}`,
      ...
      casesPerM: `${casesPerM ? casesPerM.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0}`,
      deathsPerM: `${deathsPerM ? deathsPerM.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0}`,
    })
  }
  ...
  render() {
    ...
    return (
      <tr onClick={(e) => this.clickHandle(e)} className={darkMode ? "tbody__tr tbody__tr--darkMode" : "tbody__tr"}>
        ...
        <th className="tbody__th todayCases" style={newCases !== '0' ? { backgroundColor: 'rgb(231, 94, 44)', color: '#000' } : {}}>{newCases !== '0' ? `+${newCases}` : null}</th>
        ...
        <th className="tbody__th deathRate">{this.state.deathRatio.toFixed(2)}%</th>
      </tr>
    )
  }
}
